...
/**
 * The kind of movement packet {@code LocalPlayer.sendPosition} selects for one tick, in its priority order.
 *
 * <p>It is a fact of one transition: decided by the {@link Publisher} before the tick and the {@link PlayerState}
 * after it. The server decodes it as the listener does, with absent fields falling back to the server's own values.
 */
