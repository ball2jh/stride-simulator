...
/**
 * Displacement against the world: {@code Entity.move} with its
 * {@code collide}, the phase {@link Travel} runs between acceleration and
 * drag, and the one the listener's transaction runs on a packet's
 * displacement.
 *
 * <p>In vanilla's order: the retained stuck vector consumes the request;
 * the sneaking ledge backoff; collision and step resolution; the
 * fall-distance-resetting segment test and the movement segment the block
 * effects will traverse; the new position and box; the horizontal and
 * vertical latches, ground, and the retained support; fall accounting; the
 * restitution of the blocked components with the landed block's bounce; and
 * the destination block's speed factor.
 *
 * <p>Reads the box, velocity, the stuck vector, ground, fall distance, the
 * fluid heights, and the previous support. Writes the fields in
 * {@link #WRITES} and {@link Scratch}'s moved vector and movement segment.
 * On the server's copy the vertical latches survive a request with no
 * vertical component and no fall accumulates; {@link com.nettarion.stride.simulator.server.TickAuthority#isServer()}
 * says which.
 */
public final class Move {
	...
	/**
	 * Resolve one reported player displacement through {@code Entity.move}.
	 *
	 * <p>This is the checked entry the listener's transaction uses when a
	 * movement packet supplies a displacement while the server retains its own
	 * velocity, and the one tests drive directly. It is not a complete
	 * movement tick. The requested displacement therefore
	 * remains separate from {@code state.delta*}, which the move may modify only
	 * through collision restitution and block speed factors.
	 *
	 * <p>A normal return commits the resolved center, box, contact facts, and any
	 * movement state changed by {@code Entity.move}. The same admission,
	 * completeness, explicit-refusal, and failed-call discard rules as
	 * {@link ClientTick#tick} apply.
	 *
	 ...
	 * @throws UnimplementedMechanicException when the displacement reaches an
	...
	/**
	 * {@link #resolve} for a state the simulator itself produced, whose
	 * admission the composed step established at its boundary: the same
	 * transition without re-deriving that admission on every packet.
	 */
	...
	private static void resolveUnchecked(final PlayerState state, final double requestedX, final double requestedY,
		...
		// Non-shifting movement is the identity branch of maybeBackOffFromEdge.
		// Keep that common path free of a helper call and scratch stores.
		if (shiftDown) {
		...
		if (lengthSqr > 1.0E-7
			...
			if (state.fallDistance != 0.0 && lengthSqr >= 1.0) {
				...
				double checkDistance = Math.min(length, 8.0);
		...
		// Entity.restituteMovementAfterCollisions: the isolated P3 comparison favors
		// inline arithmetic; see the package audit and build/audit-fixes2 evidence.
		if (movedVertically && state.verticalCollision || state.horizontalCollision) {
			...
			if (state.verticalCollision) {
				...
				if (state.verticalCollisionBelow && !(-currentY < PlayerTick.GRAVITY) && !shiftDown
				...
				if (restitution > 0.0) {
					// Mth.lerp(portion, 1.0, getAirDrag()), and the drag is a float
					// widened to double, not the double 0.98.
					double portionWithMovement = movedY / currentY;
					gravityCompensation = portionWithMovement * PlayerTick.GRAVITY;
					effectiveDrag = 1.0 + portionWithMovement * ((double) 0.98F - 1.0);
				} else {
	...
	/** Player.maybeBackOffFromEdge, with its scalar result in the reusable move buffer. */
		...
		if (!state.flying && !(deltaMovementY > 0.0) && shiftDown
			...
			while (deltaMovementX != 0.0
			    && Clearance.canFallAtLeast(
			        state, world, deltaMovementX, 0.0, PlayerTick.MAX_UP_STEP, shiftDown, scratch)) {
			...
			while (deltaMovementZ != 0.0
			    && Clearance.canFallAtLeast(
			        state, world, 0.0, deltaMovementZ, PlayerTick.MAX_UP_STEP, shiftDown, scratch)) {
			...
			while (deltaMovementX != 0.0 && deltaMovementZ != 0.0
			    && Clearance.canFallAtLeast(
			        state, world, deltaMovementX, deltaMovementZ, PlayerTick.MAX_UP_STEP, shiftDown, scratch)) {
				if (Math.abs(deltaMovementX) <= 0.05) {
					deltaMovementX = 0.0;
				} else {
					deltaMovementX -= stepX;
	 ...
	 *
	 * <p>Asked where the answer is read rather than beside the support search:
	 * this is the widest property span the phase asks for, and its only reader
	 * is the landing branch, which needs it on the ticks a descent actually
	 * ends against a surface.
	 */
	...
	/**
	 * {@code restituteMovementAfterCollisions}'s block term: the bounciness of the
	 * block at {@code getOnPosLegacy}, or zero when the landing cannot bounce.
	 *
	 * <p>Three ways it cannot. Too slow — the descent must be at least the
	 * effective gravity, which is what stops a resting player bouncing forever.
	 * Sneaking, through {@code isSuppressingBounce}. And the block being in
	 * {@code BlockTags.SUPPRESSES_BOUNCE}, which the capture records per cell
	 * as {@link WorldView#suppressesBounceAt}; the tag's membership is the
	 * capture's fact and is not assumed here. The caller has already excluded
	 * the first two.
	 */
	...
	/**
	 * {@code cos(MINOR_COLLISION_ANGLE)}, and a band around it wide enough that
	 * the direct comparison is certainly right outside it.
	 *
	 * <p>{@code acos} is decreasing, so {@code acos(r) < angle} is
	 * {@code r > cos(angle)} — but only exactly, and this is a bit-exact kernel.
	 * {@code Math.acos} is specified to within 1 ulp, about 3.1e-17 here, and
	 * {@code |dr/d(angle)| = sin(angle)} is about 0.139, so a disagreement needs
	 * {@code r} within roughly 4.3e-18 of the cosine; 1e-9 of slack is nine
	 * orders of magnitude more than that. Inside the band the call still runs,
	 * so the band only has to be generous, never tight.
	 */
	...
	/** LocalPlayer.isHorizontalCollisionMinor, including vanilla LUT trig. */
	...
	/**
	 * {@code Entity.collide}: collect collision shapes, resolve ordinary
	 * movement, then try step-up heights.
	 *
	 * <p>Vanilla's collider list also holds the entity collisions and, within
	 * two blocks of the world border, the border's own shape. Neither is a
	 * world fact the view carries: {@link ClientTick} admits only a state with
	 * no external entity or border collision, and that admission is the
	 * caller's to keep, so the lists here are the block shapes alone.
	 */
	private static void collide(final PlayerState state, final double dx, final double dy, final double dz,
		...
		if (PlayerTick.MAX_UP_STEP > 0.0F && (onGroundAfterCollision || state.onGround) && (xCollision || zCollision)) {
			...
			shapes = CollisionCollector.collect(world, scratch, groundMinX + Math.min(dx, 0.0), queryMinY,
			    groundMinZ + Math.min(dz, 0.0), groundMaxX + Math.max(dx, 0.0), groundMaxY + PlayerTick.MAX_UP_STEP,
			    groundMaxZ + Math.max(dz, 0.0), state.y, descending, state.fallDistance, state.canWalkOnPowderSnow);
			ShapeCollision.collectCandidateStepUpHeights(
			    groundMinY, shapes, PlayerTick.MAX_UP_STEP, (float) ordinaryY, scratch);
	...
	/**
	 * {@code Entity.checkFallDamage}. Two details that are easy to get wrong and
	 * invisible without a bit-exact gate:
	 *
	 * <p>The subtraction is {@code fallDistance -= (float) ya} — the movement
	 * is narrowed to float first, so the accumulated distance carries float
	 * resolution even though the field is a double. Subtracting the raw double
	 * agrees to about the fifteenth digit and fails.
	 *
	 * <p>The accumulate happens *before* the on-ground reset, not instead of
	 * it. On the landing tick vanilla adds the final descent and then zeroes.
	 */
