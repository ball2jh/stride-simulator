 ...
 *
 * <p>A query returns the buffer to read: the workspace's own collection when
 * the world answered it directly, or the shared {@link RetainedSpan} with the
 * groups the query box reaches selected. Callers walk the selected groups and never
 * hold a returned buffer across another query.
 */
public final class CollisionCollector {
	/**
	 * The feet a placement context reports. {@code CollisionContext.withPosition}
	 * builds an {@code EntityCollisionContext} whose {@code placement} is true,
	 * and under one {@code PowderSnowBlock.getCollisionShape} and
	 * {@code ScaffoldingBlock.getCollisionShape} answer {@code Shapes.empty()}
	 * before reading feet, shift, fall distance or boots.
	 */
	...
	/**
	 * {@code CollisionGetter.getPreMoveCollisions}: the block shapes meeting
	 * the box under a placement context, in which scaffolding and powder snow
	 * have no shape whatever the player's feet, shift key, fall distance or
	 * boots would otherwise say. Every other captured shape is context-free.
	 *
	 * <p>The listener's new-collision test is the one caller; movement itself
	 * uses {@link #collect} with the player's context. The world models both
	 * blocks' context by feet, descent, fall and boots
	 * ({@code WorldView.CollisionBehavior}), and the answers below are the
	 * ones under which each of them is empty: feet below every cell, so no
	 * cell is "above" the player; descending; no fall past the powder-snow
	 * threshold; no boots.
	 */
	...
	public static CollisionBuffer collect(final WorldView world, final Scratch scratch, final double minX,
		...
		if (!span.covers(
		        world, minX - 1.0E-7, minY - 1.0E-7, minZ - 1.0E-7, maxX + 1.0E-7, maxY + 1.0E-7, maxZ + 1.0E-7)) {
			CollisionQuerySpan.requireSupported(minX, minY, minZ, maxX, maxY, maxZ);
			// A retained span proves the world supports reuse; only a world with
			// none has to be asked.
			if (!span.names(world) && !supportsGroupedSpanReuse(world)) {
			...
			int x0 = Mth.floor(minX - 1.0E-7);
			int y0 = Mth.floor(minY - 1.0E-7);
			int z0 = Mth.floor(minZ - 1.0E-7);
			int x1 = Mth.floor(maxX + 1.0E-7);
			int y1 = Mth.floor(maxY + 1.0E-7);
			int z1 = Mth.floor(maxZ + 1.0E-7);
	...
	/**
	 * {@code Shapes.joinIsNotEmpty(shape.move(cell), Shapes.create(query), AND)}
	 * for a general source shape held as its boxes: whether some box and the
	 * query box share a merged cell that is full in both.
	 *
	 * <p>Vanilla's answer has three parts, each kept in its own arithmetic.
	 * {@code Shapes.create} makes a query thinner than {@code 1.0E-7} on any
	 * axis the empty shape, and the join is then empty. {@code joinIsNotEmpty}
	 * rejects when the whole shape's bounds and the query's are more than
	 * {@code 1.0E-7} apart on an axis. Otherwise {@code IndirectMerger}
	 * merges the two coordinate lists per axis, absorbing any coordinate
	 * within {@code 1.0E-7} of the previous one, and the join is non-empty
	 * when some merged interval lies inside a box on every axis;
	...
	/**
	 * {@code Shapes.joinIsNotEmpty}'s per-axis rejection on the whole shape's
	 * bounds: {@code first.max(axis) < second.min(axis) - 1.0E-7} or the reverse.
	 */
	private static boolean boundsMayJoin(final double shapeMinX, final double shapeMinY, final double shapeMinZ,
		...
		return !(shapeMaxX < minX - 1.0E-7) && !(maxX < shapeMinX - 1.0E-7) && !(shapeMaxY < minY - 1.0E-7)
		    && !(maxY < shapeMinY - 1.0E-7) && !(shapeMaxZ < minZ - 1.0E-7) && !(maxZ < shapeMinZ - 1.0E-7);
	}
	...
	/**
	 * {@code IndirectMerger} over the shape box's two coordinates and the
	 * query's two on one axis, for the AND join: whether the merge leaves an
	 * interval that is inside both.
	 *
	 * <p>The merger walks both lists ascending, taking the shape's coordinate
	 * while it is below the query's plus {@code 1.0E-7}, drops a coordinate
	 * that precedes the other list entirely, and folds a coordinate into the
	 * previous one when it is not more than {@code 1.0E-7} above it, which
	 * moves the previous interval's cell indices onto it. The branches below
	 * are that walk written out for two coordinates a side; the comparisons
	 * keep the merger's operand order and its placement of the epsilon.
	 */
	static boolean axisJoins(
	    final double shapeMin, final double shapeMax, final double queryMin, final double queryMax) {
		double queryMinFuzz = queryMin + 1.0E-7;
		double queryMaxFuzz = queryMax + 1.0E-7;
		if (shapeMin < queryMinFuzz) {
			...
			return shapeMax < queryMaxFuzz ? !(queryMin >= shapeMax - 1.0E-7) : !(queryMin >= queryMax - 1.0E-7);
		}
		...
		return shapeMax < queryMaxFuzz ? !(shapeMin >= shapeMax - 1.0E-7) : !(shapeMin >= queryMax - 1.0E-7);
	}
	 ...
	 *
	 * <p>A retained span proves that every shape is cell-bounded and independent of
	 * player context. The version and bounds check below preserves that proof. A
	 * miss leaves {@code out} untouched so the ordinary world query can own every
	 * unsupported case.
	 */
