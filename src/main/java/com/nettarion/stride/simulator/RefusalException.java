 ...
 */
public abstract sealed class RefusalException extends RuntimeException permits UnimplementedMechanicException,
                                                      PendingServerWriteException, UnpredictedServerWriteException,
	...
	/** A refusal with the given stable cause and explanatory message, which may be {@code null}. */
	RefusalException(final RefusalCause cause, final String message) {
		super(message);
		this.cause = Objects.requireNonNull(cause, "cause");
	}

	/** The stable classification of this refusal; the only part of a refusal a caller should compare. */
	public final RefusalCause cause() {
		return this.cause;
	}

	/** Records a stack trace only when {@code stride.refusal.trace} is set. */
	@Override
