...
/**
 * A magma block: {@code MagmaBlock.stepOn}, one point underfoot unless
 * stepping carefully, dealt by {@link ServerTick} from the step the
 * server's copy logs. It overrides no {@code entityInside}, so a visit
...
final class MagmaBlock extends BlockBehaviour {
	...
	/**
	 * {@code MagmaBlock.stepOn}: a hit unless stepping carefully, which is
	 * the synced shift flag on the server's copy; {@code Entity.hurt} is
	...
	void stepOn(final PlayerState state, final int x, final int y, final int z, final Scratch scratch) {
		if (scratch.authority.isServer() && !PlayerTick.isShiftKeyDown(state, scratch)) {
			scratch.authority.survival().hurtServer(HurtCause.HOT_FLOOR, 1.0F);
		}
	}
