...
/**
 * A slime block: {@code SlimeBlock.stepOn}, the damping of a player who
...
final class SlimeBlock extends BlockBehavior {
	...
	/**
	 * <p>Only {@code SlimeBlock} has a body that moves a client player at this
	 * pin. Magma, sculk sensor and sculk shrieker override the method and are
	 * damage-only or side-gated , so the palette calls them {@code NONE} —
	 * the fact names what has to run, not what overrides the method.
	 *
	 * <p>{@code isSteppingCarefully()} is {@code PlayerTick.isShiftKeyDown()}, which by this
	 * point in the tick is the current input on the client and the synced flag
	 * on the server's copy, the same value the pose probe reads.
	 */
	@Override
	...
	/**
	 * {@code SlimeBlock.fallOn}: unless the player suppresses the bounce, the
	 * fall at multiplier zero, which is never a hit but runs
	 * {@code causeFallDamage}'s impulse-context bookkeeping; a sneaking player
	 * lands with no fall handling at all.
	 */
	@Override
