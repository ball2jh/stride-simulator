 ...
 * hooks of vanilla's {@code BlockBehaviour} the tick can call,
 ...
 * {@link com.nettarion.stride.simulator.world.SnapshotView} compiles and asked through {@link com.nettarion.stride.simulator.world.WorldView#behaviourAt}.
 *
 ...
 * with no body at all is {@link #INERT}, which the traversal recognises by
 * identity before it dispatches. An entry whose body the slice does not
 * model is {@link UnmodelledBlock}, and one whose body depends on block
 ...
 *
 * <p>Each admitted palette tuple resolves to one upstream block behaviour.
 ...
 * be recaptured; known server-only unmodelled bodies still refuse on contact.
 */
public abstract class BlockBehaviour {
	 ...
	 */
	public static final BlockBehaviour INERT = new BlockBehaviour() {
		@Override
	...
	/** The behaviour of one palette entry. */
	public static BlockBehaviour of(final WorldSnapshot.BlockEntry entry) {
		return of(entry.bubbleColumnMode(), entry.collisionBehavior(), entry.insideEffect(), entry.stepOn(),
		    entry.contact(), entry.landing());
	}

	/**
	 * The behaviour of one tuple of palette facts. Every fact resolves to the
	 ...
	 */
	public static BlockBehaviour of(final WorldView.BubbleColumnMode bubbleColumnMode,
		...
		BlockBehaviour contacting = switch (contact) {
			case NONE -> INERT;
			...
			case LAVA_CAULDRON -> LavaCauldronBlock.INSTANCE;
			case UNMODELLED -> UnmodelledBlock.INSTANCE;
		...
		BlockBehaviour underfoot = switch (stepOn) {
			case NONE -> INERT;
			case SLIME -> SlimeBlock.INSTANCE;
		};
		BlockBehaviour landed = switch (landing) {
			case ORDINARY -> INERT;
		...
		BlockBehaviour[] owners = {bubble, powder, inside, contacting, underfoot, landed};
		BlockBehaviour single = null;
		for (BlockBehaviour owner : owners) {
			if (owner == INERT) {
				continue;
			}
			if (single == null) {
				...
				throw new UnimplementedMechanicException(Refusal.INADMISSIBLE_BLOCK_FACTS,
				    "inconsistent block behaviour facts: " + single + " and " + owner);
			}
		}
		if (single == null) {
			return INERT;
		...
		if (present != single.families() && single != UnknownStateBlock.INSTANCE
		    && single != UnmodelledBlock.INSTANCE) {
			throw new UnimplementedMechanicException(
			    Refusal.INADMISSIBLE_BLOCK_FACTS, "incomplete block behaviour facts for " + single);
		}
		return single;
	}
}
