 ...
 * {@code ServerGamePacketListenerImpl.handleMovePlayer} decoded on the
 ...
 */
public final class ServerGamePacketListenerImpl {
	/*
	...
	static void handleAcceptTeleportPacket(final ServerPlayerState server, final int id) {
		...
		if (!server.correctionPending) {
			throw new UnimplementedMechanicException(Refusal.UNMODELLED_SESSION_END,
	...
	/** ServerGamePacketListenerImpl.handlePlayerInput for an already loaded client. */
	static void handlePlayerInput(final ServerPlayerState server, final PlayerInput input) {
		server.setSharedFlag(1, input.shift());
	}

	/** ServerGamePacketListenerImpl.handlePlayerCommand's START/STOP_SPRINTING cases. */
	...
	static ServerTick.Transaction handleMovePlayer(final PlayerState clientAfter, final MovementPacket packet,
		...
		if (packet.hasPosition()
			...
			throw new UnimplementedMechanicException(
			    Refusal.UNMODELLED_SESSION_END, "invalid movement packet disconnects the client");
		}
		...
		if (server.correctionPending) {
			...
			if (server.connectionTickCount - server.awaitingTeleportTime > 20) {
				...
				result = new ServerTick.Corrected(packet, ServerTick.Cause.TELEPORT_RETRY, server.x, server.y, server.z,
		...
		if (shouldCheckPlayerMovement(server)
			...
			awaitTeleport(server);
			return new ServerTick.Corrected(packet, ServerTick.Cause.MOVED_TOO_QUICKLY, targetX, targetY, targetZ,
		...
		server.addMovementThisTick(scratch, resolvedX, resolvedY, resolvedZ);
		...
		if (fail && oldBoxFree || newCollision) {
			...
			return new ServerTick.Corrected(packet,
			    newCollision ? ServerTick.Cause.NEW_COLLISION : ServerTick.Cause.MOVED_WRONGLY, targetX, targetY,
	...
	private static void fallOn(final ServerPlayerState server, final SnapshotView world, final Scratch scratch) {
		...
		world.behaviourAt(cell.x, cell.y, cell.z)
