...
class ScheduledRecordingTest {
	...
	void serializesInterleavedEventsWithPacketsRetainedAcrossClientActions() throws Exception {
		var world = WorldSnapshot.builder(OutsidePolicy.REFUSING, -4, -2, -4, 12, 8, 12)
		...
		var trace = new ScheduledRecording(boundary, world.build(), "26.2", true, null);
		...
		trace.write(path);
		var loaded = ScheduledRecording.read(path, "26.2", null);
		...
		assertThrows(java.io.IOException.class, () -> ScheduledRecording.read(path, "wrong", null));
		...
		var disagreeing = ScheduledRecording.read(directory.resolve("corrupt.schedule"), "26.2", null);
